...
// Global variables set by imjack to override wav paths
...
// Global variables set by imjack to override wav paths
const char* g_wav_path_prefix = nullptr;
int g_row = 0;
int g_col = 0;
...
static int readfile (const char *line, int lnum, const char *cdir)
{
    ...
    if (g_wav_path_prefix) {
        const char* side = (ichan == 1) ? "l" : "r";
        snprintf(path, sizeof(path), "%s-%d-%d-%s.wav", g_wav_path_prefix, g_row, g_col, side);
    } else {
...
int config (const char *config)
{
        ...
        if (*p != '/')
	{
        ...
        else if (! strcmp (p, "/jack/client"))
	{
	    if (!jackname [0])
	    {
                if (sstring (q, jackname, NAMELEN) == 0) stat = ERR_PARAM;
	    }
	}
        else if (! strcmp (p, "/jack/server"))
	{
	    if (!jackserv [0])
	    {
                if (sstring (q, jackserv, NAMELEN) == 0) stat = ERR_PARAM;
	    }
	}
